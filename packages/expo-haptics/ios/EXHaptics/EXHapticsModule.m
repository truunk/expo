                    ...
                    reject:(EXPromiseRejectBlock)reject)
{
  ...
  if (!types[inputType]) {
    return reject(@"E_HAPTICS_INVALID_ARG", @"'type' must be one of ['success', 'warning', 'error']", nil);
  }
                    ...
                    reject:(EXPromiseRejectBlock)reject)
{
  ...
  if (!styles[inputStyle]) {
    reject(@"E_HAPTICS_INVALID_ARG", @"'style' must be one of ['light', 'medium', 'heavy']", nil);
  }
