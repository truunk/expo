...
public enum NotificationType implements VibrationType {
  SUCCESS(
      "success",
      new long[] { 0, 35, 65, 21 },
      new int[] { 0, 250, 0, 180 },
  ...
  NotificationType(String type, long[] timings, int[] amplitudes, long[] oldSDKPattern) {
    mType = type;
    mTimings = timings;
  ...
  NotificationType fromString(String type) throws IllegalArgumentException {
    for (NotificationType nt: values()) {
      if (nt.mType.equals(type)) {
        return nt;
      }
    }
    throw new IllegalArgumentException("'type' must be one of ['success', 'warning', 'error']");
  }
